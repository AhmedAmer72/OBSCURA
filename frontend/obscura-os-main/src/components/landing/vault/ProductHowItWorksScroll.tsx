...
import { ObscuraFeatureIcon } from "@/components/landing/ObscuraFeatureIcon";
import {
  getHowItWorksAccentClasses,
  type HowItWorksStep,
  type ProductHowItWorksConfig,
} from "./productHowItWorksConfigs";

const SCROLL_PER_STEP_VH = 55;

export function ProductHowItWorksScroll({ config }: { config: ProductHowItWorksConfig }) {
  const containerRef = useRef<HTMLDivElement>(null);
  const reduceMotion = useReducedMotion();
  const accent = getHowItWorksAccentClasses(config.accent);
  const stepCount = config.steps.length;
  const scroll = useProductScrollStep(containerRef, stepCount);

  if (reduceMotion) {
    return <ProductHowItWorksStatic config={config} accent={accent} />;
  }
  ...
  return (
    <section
      ...
      style={{ height: `${stepCount * SCROLL_PER_STEP_VH}vh` }}
      ...
      <div className="sticky top-0 flex min-h-screen items-center overflow-hidden">
        ...
        <div className="relative mx-auto flex w-full max-w-[1200px] flex-col gap-10 px-6 py-16 md:px-8 lg:flex-row lg:items-center lg:gap-16 lg:py-20">
          <StepRail
            config={config}
            accent={accent}
            activeStep={scroll.activeStep}
            lineScale={scroll.lineScale}
          />
          <StepStage config={config} accent={accent} activeStep={scroll.activeStep} />
        </div>
...
function useProductScrollStep(containerRef: RefObject<HTMLElement | null>, stepCount: number) {
...
type AccentClasses = ReturnType<typeof getHowItWorksAccentClasses>;
...
function StepRail({
  ...
  config: ProductHowItWorksConfig;
  ...
  return (
    <div className="lg:w-[42%] lg:shrink-0">
      <p className={cn("font-mono text-[10px] uppercase tracking-[0.24em]", accent.label)}>
        ▸ How it works
      </p>
      <h3 className="mt-3 font-display text-3xl leading-tight tracking-tight md:text-4xl">
        {config.headline}
        <br />
        <span className="text-white/55">{config.headlineMuted}</span>
      </h3>
      <p className="mt-4 max-w-sm text-sm leading-relaxed text-white/50">{config.description}</p>

      <ol className="relative mt-10 space-y-0">
        <div className="absolute bottom-2 left-[11px] top-2 w-px bg-white/12" aria-hidden />
        <motion.div
          className={cn("absolute bottom-2 left-[11px] top-2 w-px origin-top", accent.line)}
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 pt-0.5">
                ...
                {isActive ? (
                  <p className="mt-1 text-xs leading-relaxed text-white/45 lg:hidden">{step.brief}</p>
                ) : null}
              </div>
            </li>
          );
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="flex items-start justify-between gap-4">
            ...
            <span className={cn("font-mono text-[10px] uppercase tracking-[0.2em]", accent.label)}>
              {String(current + 1).padStart(2, "0")} / {String(stepCount).padStart(2, "0")}
            </span>
          </div>
          ...
          <div className="mt-8 rounded-xl border border-dashed border-white/20 bg-white/5 px-4 py-3">
            <p className="font-mono text-[10px] uppercase tracking-wider text-white/40">In the app</p>
            <p className={cn("mt-1 font-mono text-sm", accent.hint)}>{step.hint}</p>
          </div>

          <StepMiniFlow steps={config.steps} accent={accent} activeIndex={current} />
        </motion.article>
      </AnimatePresence>
    </div>
  );
            ...
            {i < steps.length - 1 ? (
              <div
                className={cn(
                  ...
                  i < activeIndex ? accent.miniDone : "bg-white/12",
                )}
              />
            ) : null}
          </div>
...
function ProductHowItWorksStatic({
  config,
  accent,
}: {
  config: ProductHowItWorksConfig;
    ...
    <section className="bg-forest px-6 py-20 text-white md:px-8">
      <div className="mx-auto max-w-[1200px]">
        ...
        <h3 className="mt-3 font-display text-3xl tracking-tight">
          {config.headline} {config.headlineMuted}
        </h3>
        <ul className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {config.steps.map((step, i) => (
            <li key={step.id} className="rounded-2xl border border-white/12 bg-white/[0.06] p-5">
